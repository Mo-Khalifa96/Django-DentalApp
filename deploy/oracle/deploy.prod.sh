...
#Export domain name to nginx conf files
...
#Pull new code from github (using update script)
if [[ "${SCRIPT_ARGS[*]}" == *"--enable-rollback"* ]]; then
    ./update.sh
fi
...
if [[ "${SCRIPT_ARGS[*]}" == *"--rebuild"* && "${SCRIPT_ARGS[*]}" == *"--no-cache"* ]]; then
    ...
    echo "Building production imag with production dependencies..."
    docker compose -f docker-compose.prod.yml build --build-arg ENVIRONMENT=production
else
...
if docker compose -f docker-compose.prod.yml run --rm --entrypoint certbot certbot certificates 2>/dev/null | grep -q "INVALID\|will expire"; then
...
if ! docker compose -f docker-compose.prod.yml exec -T nginx ls /etc/letsencrypt/live/${SITE_DOMAIN}/ >/dev/null 2>&1; then
...
if ! curl -f --connect-timeout 10 --max-time 30 https://${SITE_DOMAIN}/health/; then
    echo "HTTPS access test failed! Stopping deployment..."
    exit 1
fi
echo ""
echo ""
...
#For enabling git rollback (revert to pre-deployment state):
  # ./deploy.prod.sh --enable-rollback  # or,
  # ./deploy.prod.sh --rebuild --enable-rollback  # or,
  # ./deploy.prod.sh --rebuild --no-cache --enable-rollback
